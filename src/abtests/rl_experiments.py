...
df_orgs = df_orgs[
    (df_orgs["sent_time"] >= date - timedelta(days=60)) & (df_orgs["sent_time"] <= date + timedelta(days=60))
]
...
n_messages_by_guardian_dict_before = (
    df_orgs[df_orgs["sent_time"] < date].groupby("guardian_id")["interactions_id"].apply(len).to_dict()
)
n_messages_by_guardian_dict_after = (
    df_orgs[df_orgs["sent_time"] >= date].groupby("guardian_id")["interactions_id"].apply(len).to_dict()
)
...
T_bin = df[
    ["Treatment - Sample 1", "Treatment - Sample 2", "Treatment - Sample 3"]
...
def treat_map(t):
    return np.dot(t, np.arange(1, t.shape[0] + 1))
