...
group.add_argument(
    "--random_groups", "-rg", help="Number of random group ids to do analysis", type=int,
)
group.add_argument(
...
def group_plot(group_id, mi, yi, mf, yf, data=df):
    ...
    # if first value is nan, subtract 1 from CDVs as not to count nans as a valid guardian id
    #if np.isnan(df_group["guardian_id"].values[0]):
    #    cumulative_new_guardians -= 1
    ...
    plt.savefig(
        f"outputs/plots/group_{group_id}-mi_{mi}-yi_{yi}-mf_{mf}-yf_{yf}-cumulative_new_guardians.png"
    )
    ...
    plt.savefig(
        f"outputs/plots/group_{group_id}-mi_{mi}-yi_{yi}-mf_{mf}-yf_{yf}-cumulative_messages.png"
    )
    ...
    plt.legend(bbox_to_anchor=(1.125,1.15),loc="upper right")
    plt.savefig(
        f"outputs/plots/group_{group_id}-mi_{mi}-yi_{yi}-mf_{mf}-yf_{yf}-cumulative_individual_messages.png"
    )

if args.group_id:
    group_plot(args.group_id, args.initial_month, args.initial_year, args.final_month, args.final_year)
...
if args.random_groups:
    for gid in random.sample(list(df["groups_id"].unique()), args.random_groups):
        group_plot(gid, args.initial_month, args.initial_year, args.final_month, args.final_year)
